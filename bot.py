...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [
            ...
            InlineKeyboardButton("Timeframe", callback_data="timeframe"),
        ],
        [
...
"""
    ...
    keyboard = [
        [
            InlineKeyboardButton("Accounts", callback_data="account_type"),
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                f"{'✅' if current == 'spot' else '◾'} Spot (trading)",
                callback_data="at_spot",
            )
        ],
        [
            InlineKeyboardButton(
                f"{'✅' if current == 'margin' else '◾'} Margin",
                callback_data="at_margin",
            )
        ],
        [
            InlineKeyboardButton(
                f"{'✅' if current == 'futures' else '◾'} Futures",
                callback_data="at_futures",
            )
        ],
    ...
    for i in range(0, len(state.TOP_PAIRS), 2):
        ...
        for j in range(2):
            if i + j < len(state.TOP_PAIRS):
                ...
                row.append(
                    InlineKeyboardButton(
                        f"{symbol} {pair.split('/')[0]}", callback_data=f"pair_{pair}"
                    )
                )
        if row:
            keyboard.append(row)
...
async def show_signals_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [
            InlineKeyboardButton(
                f"Interval ({signal_interval}m)", callback_data="signal_interval"
            )
        ],
        [
            InlineKeyboardButton(
                f"Pairs ({len(signal_pairs)})", callback_data="signal_pairs"
            )
        ],
        [InlineKeyboardButton("Back", callback_data="back")],
    ]
...
async def show_signal_interval(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                f"{'✅' if current == 5 else '◾'} 5 minutes", callback_data="si_5"
            )
        ],
        [
            InlineKeyboardButton(
                f"{'✅' if current == 15 else '◾'} 15 minutes", callback_data="si_15"
            )
        ],
        [
            InlineKeyboardButton(
                f"{'✅' if current == 30 else '◾'} 30 minutes", callback_data="si_30"
            )
        ],
        [
            InlineKeyboardButton(
                f"{'✅' if current == 60 else '◾'} 1 hour", callback_data="si_60"
            )
        ],
        [InlineKeyboardButton("Main menu", callback_data="back")],
    ]
...
async def show_timeframes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for tf_key, (tf_name, _) in state.TIMEFRAMES.items():
        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    f"{is_selected} {tf_name}", callback_data=f"tf_{tf_key}"
                )
            ]
        )
...
async def show_api(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [
            InlineKeyboardButton(
                f"Perplexity {perplexity_status}", callback_data="add_perplexity"
            )
        ],
...
async def show_ai(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [
            InlineKeyboardButton("What to buy?", callback_data="ai_buy"),
            InlineKeyboardButton("Indicators", callback_data="ai_indicators"),
        ],
        [
            InlineKeyboardButton("Strategy", callback_data="ai_strategy"),
            InlineKeyboardButton("Risk", callback_data="ai_risk"),
        ],
        [InlineKeyboardButton("Main menu", callback_data="back")],
    ]
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            WAITING_SECRET: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, receive_secret_key)
            ],
            WAITING_PASSPHRASE: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, receive_passphrase)
            ],
            WAITING_PERPLEXITY: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, receive_perplexity_key)
            ],
        },
